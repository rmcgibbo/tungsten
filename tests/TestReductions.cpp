...
int test_MPIcsAdd(const std::string& method) {
    ...
    const int SIZE = MPI::COMM_WORLD.Get_size();
    const int RANK = MPI::COMM_WORLD.Get_rank();
    const int N = 3; // the size of the matrix
    const int K = 7; // the number of entries on each rank
    ...
    if (RANK == MASTER) {
        ...
        for (int i = 0; i < expected->m; i++)
            for(int j = 0; j < expected->n; j++)
                ASSERT_EQUAL(cs_get(expected, i, j), cs_get(result, i, j));
